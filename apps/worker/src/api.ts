...
export async function handleApi(request: Request, env: Env, identity: Identity): Promise<unknown> {
  ...
  if (resource === "test-email" && !id && request.method === "POST") {
    const poems = await db<Poem[]>(env, "poems", { query: { select: "id,user_id,title,author,body,language,active,access_type,source_type,source_title,source_section,source_page,source_url,rights_note,attribution_year,explainer", user_id: "is.null", active: "eq.true", order: "created_at.asc", limit: "1" } });
    const poem = poems[0];
