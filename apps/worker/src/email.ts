...
export async function sendEmail(env: Env, profile: Profile, poem: Poem | null, birthday?: Birthday): Promise<string> {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from: env.FROM_EMAIL, to: [profile.email], subject, html: `<div style="font-family:Georgia,serif;max-width:620px;margin:auto;color:#28241f">${intro}${poemHtml}<p style="margin-top:32px;color:#81776d">Poetic Reminder</p></div>` }),
  });
